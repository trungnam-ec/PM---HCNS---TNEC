...
export default function VoiceSampleManager({
  ...
  const saveSample = useCallback(async (
    ...
    try {
      ...
      if (!rows || rows.length === 0) {
        throw new Error("Tài khoản của bạn không có quyền sửa hồ sơ nhân sự nên mẫu giọng không được lưu. Nhờ Phòng HCNS ghi giúp.");
      }
      onChanged();
    } catch (err: any) {
  ...
  const deleteSample = useCallback(async (employeeId: string, path: string) => {
    ...
    try {
      ...
      if (dbErr) throw dbErr;
      if (!rows || rows.length === 0) {
        throw new Error("Tài khoản của bạn không có quyền sửa hồ sơ nhân sự nên trạng thái mẫu giọng chưa được cập nhật.");
      }
      onChanged();
    } catch (err: any) {
